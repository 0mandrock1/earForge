...
async function loadP(){try{const v=localStorage.getItem(SK);return v?JSON.parse(v):null}catch(e){return null}}
async function saveP(d){try{localStorage.setItem(SK,JSON.stringify(d))}catch(e){}}
async function loadSkips(){try{const v=localStorage.getItem(TK);return v?JSON.parse(v):{}}catch(e){return{}}}
async function saveSkips(d){try{localStorage.setItem(TK,JSON.stringify(d))}catch(e){}}
...
function DiffPicker({modeId,onPick}){
  ...
  return <div className="flex-1 flex flex-col items-center justify-center px-4 gap-5 pb-8" style={{animation:"fadeIn .3s"}}>
    ...
    <div className="flex flex-col gap-3 w-full max-w-xs">
      {DIFFS.map(d=>(
        <button key={d.id} onClick={()=>onPick(d.id)} className="flex items-center gap-3 p-4 rounded-xl text-left hover:scale-[1.02] active:scale-95 transition-transform" style={{backgroundColor:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.1)"}}>
