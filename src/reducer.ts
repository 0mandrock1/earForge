...
export function reducer(st:typeof initState,a:any){
  switch(a.type){
    case "GO":return{...st,screen:a.screen,sessionWeak:!!a.weak};
    ...
    case "CORRECT":{
      ...
      const mode=a.mode||st.screen,ms=(st.stats as any)[mode]||{ok:0,total:0};
    ...
    case "WRONG":{const mode=a.mode||st.screen,ms=(st.stats as any)[mode]||{ok:0,total:0};
