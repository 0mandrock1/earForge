  ...
  return(
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{backgroundColor:"rgba(0,0,0,.75)"}}>
      <div className="w-full max-w-sm mx-4 rounded-2xl p-5 flex flex-col gap-4"
        style={{backgroundColor:"#1c0808",border:"1px solid rgba(239,68,68,.4)",maxHeight:"80vh",overflowY:"auto"}}>
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-red-300 hover:text-white text-lg px-2 py-1">✕</button>
        </div>
        {!data&&<div className="text-red-400 text-sm text-center py-4">...</div>}
        {data&&MODES_META.map(m=>{
          ...
          return(
            <div key={m.id}>
              ...
              {entries.length===0
                ?<div className="text-red-400 text-xs pl-1">{t.ui.noEntries}</div>
                :<div className="flex flex-col gap-1">
                  {entries.map((e,i)=>(
                    <div key={e.nick} className="flex items-center gap-2 px-3 py-2 rounded-xl"
                      style={{backgroundColor:i===0?"rgba(251,191,36,.12)":e.nick===currentNick?"rgba(239,68,68,.15)":"rgba(255,255,255,.06)"}}>
                      <span className="font-bold text-sm w-5" style={{color:i===0?"#fbbf24":i===1?"#d1d5db":"#f87171"}}>{i+1}.</span>
                      <span className="text-sm flex-1 truncate" style={{color:e.nick===currentNick?"#fca5a5":"white"}}>{e.nick}{e.nick===currentNick?" ◀":""}</span>
                      <span className="text-green-400 text-xs font-bold">{e.pct}%</span>
                      <span className="text-xs" style={{color:"#fbbf24"}}>🔥{e.best}</span>
                      <span className="text-red-400 text-xs">{e.ok}/{e.total}</span>
                    </div>
                  ))}
                </div>
              }
