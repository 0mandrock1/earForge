...
function newKeyRound(diff:string,exclude:string|null=null,weakOnly?:boolean){
  const keyPool=diff==="easy"?NOTES.map(k=>k+" maj"):NOTES.flatMap(k=>[k+" maj",k+" min"]);
  const nOpts=diff==="easy"?3:diff==="medium"?4:6;
  const base=weakOnly?keyPool.filter(l=>getWeakKeys("key").includes(l)):keyPool;
  const src=base.length>0?base:keyPool;
  const fp=exclude?src.filter(l=>l!==exclude):src;
  const label=weightedPick("key",fp.length>0?fp:src,x=>x,()=>randBuf.get());
...
export default function KeyMode({audio,dispatch,streak,diff,onAdvance,weakOnly}:any){
  const t=useT();const fb=useGameFB(streak,dispatch,"key");
  const lastAnsRef=useRef<string|null>(null);
  const nextRound=useCallback(()=>{const n=newKeyRound(diff,lastAnsRef.current,weakOnly);lastAnsRef.current=n.ans;return n;},[diff,weakOnly]);
  ...
  const goNext=()=>{
    ...
    const n=nextRound();setR(n);setLk(false);fb.reset();setTimeout(()=>audio.playProgression(n.chords,.7),100);
  };// eslint-disable-line
